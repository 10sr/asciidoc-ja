...
AsciiDoc は、TODO notes 、文書、記事、書籍、 ebook 、スライド、ウェブページ、
ブログ、 UNIX の man ページを書くための文書フォーマットです。 AsciiDoc
で書かれたファイルは HTML, PDF, EPUB, man ページなど様々なフォーマットに
変換することができます。
AsciiDoc is highly configurable: both the
...
.この文書について
**********************************************************************
この文書はかなり巨大であり、チュートリアル、クイック・リファレンス、
リファレンスの３つに整理する必要があります。

もしあなたが AsciiDoc を初めて使おうと思ってこの文章を読んでいるなら、
このセクションと <<X6,Getting Started>> セクションを読み、
TODO: 配布物の `doc` ディレクトリにあるサンプルの AsciiDoc (`*.txt`)
ファイルを眺めてみると良いでしょう。
**********************************************************************
...
AsciiDoc フォーマットそれ自体使いやすく見やすいフォーマットです。
AsciiDoc のマークアップはシンプル・直感的で、容易に校正や編集ができます。

AsciiDoc は軽量であり、単一の Python スクリプトと TODO 設定ファイル群で
構成されています。
asciidoc(1) と Python インタプリタのみを用意すれば AsciiDoc ファイルから
DocBook や HTML を生成することができます。
以下の <<X11,Example AsciiDoc Documents>> を見てください。

文書のマークアップのやり方は、個人の強い好みによることが多いです。
もしデフォルトのシンタックスが好みに合わなければ、 asciidoc(1) 設定ファイルを
書き換えることで自分の好きなシンタックスを定義できます。
TODO: また、新たに設定ファイルを作成すれば、 AsciiDoc を大抵の SGML/XML マークアップに
変換するようにすることもできます。
...
Block Elements
~~~~~~~~~~~~~~
ブロック要素 (Block elements) は、一行以上のテキストやその他のブロック要素で構成されます。

AsciiDoc のブロック構造は、おおよそ以下のようにまとめられます。
footnote:[これは概要であり、厳格なシンタックスの定義ではありません]
...
- '?' は TODO: ゼロか一度の出現、 '+' は一度以上の出現、
  '*' はゼロかそれ以上の出現を表す
- すべてのブロック要素は TODO: line boundaries によって区切られる
- ここにリストされていないブロック要素 `BlockId` 、 `AttributeEntry` と
  `AttributeList` はほとんどどこにでも置くことができる
- TODO: There are a number of document type and backend specific
  restrictions imposed on the block syntax.
- 次の要素は空行を含むことができない： Header, Title, Paragraph, ItemText
- TODO: A ListParagraph is a Paragraph with its 'listelement' option set.
- ListContinuation は <<X15,list continuation element>> である

[[X95]]
Header
~~~~~~
ヘッダは、タイトルや著者・リビジョン情報などのメタデータを含むことができます。

- ヘッダは書かなくても構いませんが、書く場合文書の <<X17,title>>
  ではじめる必要があります
- 著者とバージョン情報を書く場合、それはタイトルの直後である必要があります
- 文書のヘッダとそれ以外の部分の間には一つ以上の空行が必要であり、また
  ヘッダには空行を含めることはできません
- ヘッダにはコメントを含めることができます
- ヘッダには次のような <<X18,attribute entries>> を含めることができます：
  'doctype', 'lang', 'encoding', 'icons', 'data-uri', 'toc',
  'numbered'.
- ヘッダに記述された属性はコマンドライン引数によって上書きすることができます
- TODO: ヘッダに UTF-8 でない文字を含む場合、 'encoding' はヘッダより前に置く必要が
  あります（ドキュメント内またはコマンドライン引数）

AsciiDoc の文書のヘッダの例は以下のようになります：
...
著者の情報のあとには、 E メールアドレスを続けることができます。
著者情報の書式は以下のようになります：

  firstname[ [middlename ]lastname][ <email>]]

つまり、ファーストネーム・ミドルネーム・ラストネーム・ E メールアドレスの順序となります。
ミドルネーム・ラストネーム・ E メールアドレスは必須ではありません。
ファーストネーム・ミドルネーム・ラストネームが複数の単語からなる場合、アンダースコア
で語を区切る事ができます。
E メールアドレスは全体を <> でくくる必要があります。
以下に著者情報の例を載せます：
...
著者情報の行が上記の書式に従っていない場合行全体がファーストネームと解釈されます。

著者情報の行に続けて、リビジョン情報を追加することができます。
Revision 情報は以下の書式のどちらかになります。

. ドキュメントのリビジョン番号・そのリビジョンの日時・そのリビジョンに関する注釈を
  この順に順に記述します。
  これらの要素はすべて必須ではありません
+
--
  * リビジョン番号が指定された場合末尾にコンマを付加する必要があります
  * リビジョン番号のうち最低でも一文字は数字である必要があります
  * はじめの数字より前にある数字以外の文字はすべて取り除かれます
  * リビジョンに関する注釈が与えられた場合、それはコロンで始まる必要があります。
    次に現れる空行・ TODO: attribute entry またはコメントまでが注釈として扱われます。
    TODO: この注釈に対しては、通常のテキスト置換が行われます。
  * リビジョン番号または注釈が存在し、リビジョンの日時が与えられてしない場合、
    日時には 'docdate' 属性の値が用いられます
...
. リビジョン情報はまた、 RCS/CVS/SVN の $id$ キーワードを用いることもできます：
+
--
  * AsciiDoc は、 'revnumber', 'revdate', 'author' の属性値を TODO: $Id$ revision marker
    から取り出し、文書のヘッダに表示します
  * もし TODO: $Id$ revision marker が用いられた場合ヘッダの著者の行は
    省略できます
